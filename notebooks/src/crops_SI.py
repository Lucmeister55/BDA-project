# Copy from pbmc3k_SI.py with slight changes
# Would it be better to have one SI file that works for all examples? Is it specifically called pbmcd3k because it should only work on that example?
...
# -------------------------------
# 0. K-Means Implementation (baseline)
# -------------------------------
def run_kmeans(data_tsne, n_clusters, n_init=10, max_iter=300):
    kmeans = KMeans(
        n_clusters=n_clusters, n_init=n_init, max_iter=max_iter, random_state=42
    )
    kmeans.fit(data_tsne)
    labels = kmeans.labels_
    best_centers = kmeans.cluster_centers_

    # Compute cost manually to match the other methods
    distances = cdist(data_tsne, best_centers, metric="euclidean")
    min_distances = np.min(distances, axis=1)
    best_cost = np.sum(min_distances**2)
...
def run_abc(
    ...
    if record_history:
        history = {"positions": [food_sources.copy()], "costs": [best_cost]}
    else:
        history = None
...
def run_acor(
    ...
    if record_history:
        ...
        archive_history = {"archives": [archive.copy()], "costs": [best_cost_initial]}
    else:
...
def animate_sa(data_tsne, pos_history, centers, interval=200, figsize=(8, 6), margin=5):
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=len(pos_history), interval=interval, blit=True, repeat=False
    )
    return ani
